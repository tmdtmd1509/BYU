...
public:
	...
	void printTuples(Predicate predicate, string name);
	...
	Tuple joinTuple(const Tuple &t1, const Tuple &t2, const vector<int> &uniqueCols);

	bool isJoinable(const Tuple &t1, const Tuple &t2, const vector<pair<int, int>> &matchingCols);
